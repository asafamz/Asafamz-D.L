...
export async function retrieveWhopProduct(productId: string) {
  return whopFileRequest(`/products/${encodeURIComponent(productId)}`);
}

export async function updateWhopProductMetadata(productId: string, metadata: Record<string, unknown>) {
  return whopFileRequest(`/products/${encodeURIComponent(productId)}`, {
    method: "PATCH",
